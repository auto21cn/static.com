	...
	var Mapplic = function() {
		...
		function Deeplinking() {
			...
			this.check = function(ease) {
				...
				// tool tip on retail detail needs
				// locationid: 'coach' attribute
				var store_id = self.o.locationid;
				if (store_id && store_id !== '') { id = store_id; }
		...
		function FullScreen() {
			...
			this.init = function() {
				...
				/* HTML5 fullscreen is not supported in all browsers - prevent drawing button if not supported */
				var fullScreen = document.fullscreenEnabled || document.webkitFullscreenEnabled || document.mozFullScreenEnabled || document.msFullscreenEnabled ? true : false;
				if (fullScreen) {
		...
		var processData = function(data) {
			...
			$(window).resize(function() {
				// Mobile
				if ($(window).width() < 668) {
					var orientationVal = (window.matchMedia('(orientation: portrait)').matches) ? 150 : 66;
					self.container.height($(window).height() - orientationVal);
				}
